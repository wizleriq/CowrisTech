import React from 'react'

const Page = () => {
  return (
    <section className="w-full max-w-[1440px] mx-auto lg:h-[585px] lg:bg-green-500 lg:flex lg:flex-row lg:justify-center lg:items-center">
      
      {/* Left box */}
      <div className="lg:w-full max-w-[1440px] lg:h-[585px] lg:bg-[#b4e5f3] lg:p-16">
        {/* Content for left box */}
      </div>

      {/* Right box */}
      <div className="lg:w-full max-w-[1440px] lg:h-[585px] lg:bg-[#F8FAFC] lg:flex lg:justify-center lg:items-center">
        {/* Content for right box */}
      </div>

    </section>
  )
}

export default Page








// import React from 'react'

// const page = () => {
//   return (
//         // <section className="w-full lg:h-[585px] max-w-[1440px] lg:bg-green-500 lg:flex lg:flex-row lg:justify-center lg:items-center">
//            <section className="lg:w-full lg:h-[585px] max-w-[2000px] lg:bg-green-500 lg:flex lg:flex-row lg:justify-center lg:items-center">

//            <div className='lg:w-[690px] lg:h-[585px] lg:bg-[#FFFFFF] lg:p-16'>

//             </div>
//           <div className='lg:w-[710px] lg:h-[585px] lg:bg-[#F8FAFC] lg:flex lg:justify-center lg:items-center '>
//             </div>
//     </section>
    
//   )
// }

// export default page

// width: 959;
// height: 1024;
// angle: 0 deg;
// opacity: 0.48;
